...
class CentralController:
    ...
    def TargetPlanner(self,matrix,state,robot,entrance):
        ...
        if(cnt_valid_robot<=cnt_require_entrance_urgent):
            ...
            for i in range(len(valid_robot_list)):
                for j in range(len(entrance_urgent)):
                    matrix[i,j]=self.cost(self.robotList[i],self.entranceList[j])
            self.TargetPlanner(matrix,0,valid_robot_list,entrance_urgent)
            ...
            for i in range(len(valid_robot_list)):
                for j in range(len(entrance_urgent)):
                    matrix[i,j]=self.cost(self.robotList[i],self.entranceList[j])
            self.TargetPlanner(matrix,1,valid_robot_list,entrance_urgent)
